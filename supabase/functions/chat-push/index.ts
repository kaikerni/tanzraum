...
// POST -> { nachricht_id } vom Datenbank-Trigger (Header x-tanzraum-geheimnis). Empfaenger bestimmt
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!geheimnis || typeof nachricht_id !== "string") return new Response("Ungültige Anfrage", { status: 400 });
    const paar = await vapidPaar(admin);
    if (!paar) return new Response(JSON.stringify({ ok: false, grund: "VAPID fehlt" }), { status: 200 });
    const { oeffentlich, privat } = paar;
    const { data: ziele, error } = await admin.rpc("chat_push_ziele", { p_geheimnis: geheimnis, p_nachricht_id: nachricht_id });
